...
class taskSeries {
  ...
  execute(jm, sid) {
    ...
    this.sid = sid; //FIXME is this.sid used?
    return new Promise((resolve, reject) => {
      jm.task._queue.process(sid, (job, done) => {
        debug('execute process job.data: %o', job.data);
        if (job.data.status === 'complete' && job.data.result) { //FIXME we don't need this
          ...
          mod(JSON.parse(param), input, (err, res) => {
            if (err) {
              ...
              if (job.data.rewindFlg) {
                ...
                delay(100)
                  .then(() => this._processRewind(sid, Number(job.data.idx), input))
                  .then(() => this.jm.cleanWorker(sid))
      ...
      return this.createStep(jm, sid, 0)
        .then(res => resolve(res)) //FIXME no needed
          ...
          job
            .on('complete', (result) => {
              ...
              jm.job._db
                ...
                .exec()
                .then(() => {
                  return this.next(jm, sid, idx); //FIXME why found next here? what's if it's the last task?
                })
                .then((nextTask) => {
   ...
   * @private
   */
  _processRewind(sid, errIdx, input) {
    debug('_processRewind sid: %s \t redIdx: %d \t input: %o', sid, errIdx, input);
    let curIdx = errIdx;
    // init rewind tasks
      ...
      Promise.all(rewindTasks)
        .then(res => {
          ...
          if (tasks.length === 0) {
            ...
            return multi.exec()
              .then(() => resolve(this.createStep(this.jm, sid, errIdx + 1, input)));
          }
        })
        .catch((err) => reject(err));
    });
