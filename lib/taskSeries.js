...
class taskSeries {
   ...
   * @param {number} ttl for each task key in redis
   * @return
   */
  execute(jm, sid, taskTTL) {
    ...
    return this.createStep(jm, sid, 0)
      .catch((err) => {
        // TODO log err
        return this._processRewind(sid, err.index, taskTTL)
          .catch((e) => {
   ...
   * @private
   */
  _processRewind(sid, errIdx, taskTTL = 3600 * 24 * 180) {
        ...
        this.jm.job._db.hgetall(`${sid}:${idx}`)
          .then((task) => {
            ...
            if (task.rewindHandler) {
              task.rewindFlg = true;
              task.idx = ++curIdx;
              const record = multi.hmset(`${sid}:${curIdx}`, task);
              multi.expire(`${sid}:${curIdx}`, taskTTL);
              return record;
            }
          })
      );
