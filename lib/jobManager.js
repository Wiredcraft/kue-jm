...
class JobManager extends EventEmitter {
  ...
  constructor(opt) {
    ...
    this.task = { _queue: redisFactory.task(opt), collections: new Map() }; // task is no more needed, _queue should be fine enough
  ...
  run(type, concurrency = 1) {
    ...
    return new Promise((resolve, reject) => { //FIXME no need to wrap it with Promise, the `run` is more likely a listener
      if (!type || typeof type !== 'string') {
      ...
      this.task._queue.process(type, concurrency, (job, done) => {
        ...
        multi.exec((err) => {
          ...
          series.execute(this, sid)
            ...
            .catch((err) => {
              debug('execute err: %o', err); //FIXME no clean if error?
              done(err);
            });
          resolve(sid);
        });
