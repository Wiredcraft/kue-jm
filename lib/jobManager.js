...
class JobManager extends EventEmitter {
      ...
      .then(() => {
        ...
        task
          ...
          .save((err) => {
            if (err) {
              jobDb.del(key);
              return Promise.reject(new Error(`save job error, ${err.stack}`));
            }
   ...
   * @param {number} ttl for each job key in redis
   * @return {string} job uuid
   */
  run(type, concurrency = 1, taskTTL = 3600 * 24 * 180) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.task._queue.process(type, concurrency, (job, done) => {
        ...
        multi.exec((err) => {
          ...
          series.execute(this, sid, taskTTL)
            .then((res) => {
  ...
  cleanWorker(type) {
    ...
    if (this.task._queue.workers) {
      ...
      if (idx !== -1) {
        this.task._queue.workers[idx].shutdown();
  ...
  registerHandler(name, handler) {
    debug('registerHandler name: %s', name);
    if (this.handlers.get(name)) {
      throw new Error('this handler name is already token');
    }
    this.handlers.set(name, handler);
  }

  setGracefulShutdown(timeout = 5000) {
    process.once('SIGTERM', () => {
      this.tasks._queue.shutdown(timeout, (err) => {
        debug('Kue shutdown with err %j', err);
        process.exit(0);
      });
    });
  }
